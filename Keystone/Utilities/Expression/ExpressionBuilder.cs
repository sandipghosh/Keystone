...
namespace Keystone.Web.Utilities.Expression
{
    using System;
    ...
    {
        public static Expression<Func<T, TResult>> BuildLamdaExpression<T, TResult>(string stringExpression)
        {
        ...
        public static Expression<Func<T1, T2, TResult>> BuildLamdaExpression<T1, T2, TResult>(string stringExpression)
        {
            if (stringExpression == null || stringExpression == string.Empty) return null;
        ...
        {
            Func<T, TResult> functor = BuildLamdaExpression<T, TResult>(stringExpression).Compile();
            return functor;
        }
        ...
        {
            Func<T1, T2, TResult> functor = BuildLamdaExpression<T1, T2, TResult>(stringExpression).Compile();
            return functor;
        }
                    ...
                    case 0:
                        {
                            ...
                            if (mcToken != null)
                            {
                                if (mcToken.IsString)
                                {
                                    ...
                                    {
                                        DateTime dt;
                                        if (DateTime.TryParse(mcToken.StrValue, out dt))
                                            result = Expression.Constant(dt);
                                        else
                                            result = Expression.Constant(mcToken.StrValue);
                                    }
                    ...
                    case 2:
                        {
                            ...
                            if (brToken != null)
                            {
                                ...
                                if (methodToken != null)
                                {
                                    MethodInfo mi = TypeUtils.GetMethodInfo(result.Type, methodToken.StrValue, argsCount);
                                    ...
                                    else
                                    {
                                        ...
                                        if (TypeUtils.IsExtensionMethod(result.Type, mi))
                                            result = Expression.Call(null, mi, result, args[0]);
                                        else
                                            Expression.Call(result, mi, args);
                                    }
                                }
